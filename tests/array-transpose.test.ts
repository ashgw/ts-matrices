...
test('transposes array with complex types and handles uneven rows', () => {
  const result: TestType<
    Transpose<[[1, Optional<Numeric>, Nullable, 3], [4, 5, 6]]>,
...
test('handles single element array', () => {
  const result: TestType<Transpose<[[1]]>, [[1]], true> = true;
  expect(result).toBe(true);
});

test('transposes array with mixed number and boolean types', () => {
  const result: TestType<
    Transpose<[[1, true, 3], [4, 5, 6]]>,
    [[1, 4], [true, 5], [3, 6]],
    true
  > = true;
  expect(result).toBe(true);
...
test('handles empty array', () => {
  const result: TestType<Transpose<[]>, [], true> = true;
  expect(result).toBe(true);
});

test('transposes array with single row', () => {
  const result: TestType<Transpose<[[1, 2, 3]]>, [[1], [2], [3]], true> = true;
  expect(result).toBe(true);
});
